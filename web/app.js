...
function setResult(result){
  ...
  $('modeBadge').textContent=result.mode==='historical'?'历史记录回放':'真实 API 记录';
...
async function loadDemo(){if(S.job)return toast('请先等待当前任务完成。');try{S.demo=window.__BOOT__||S.originalDemo||await fetch('/demo.json').then(r=>r.json());S.originalDemo=S.demo;S.text='';hydrateGraph(S.demo.graph);$('presets').hidden=false;$('presets').innerHTML=S.demo.results.map((r,i)=>`<option value="${i}">${i+1}. ${esc(r.question)}</option>`).join('');selectPreset(0);}catch(e){toast(e.message);}}
function selectPreset(i){const r=S.demo.results[i];$('question').value=r.question;setResult(r);}
...
function estimate(){const chars=$('novelText').value.length,size=+$('chunkSize').value||5000;const n=Math.ceil(chars/Math.max(1,size-300));$('estimate').textContent=`${chars.toLocaleString()} 字符 · 预计约 ${n}–${Math.ceil(n*1.5)} 个块，实际按句末切分。约每块 1 次建图请求 + 每问 2 次请求。`;}
...
$('ask').onclick=async()=>{
  ...
  if(!S.config){const old=S.demo?.results.find(r=>r.question===q);if(old&&S.graph?.meta?.mode==='historical'){setResult(old);S.playing=true;return;}return toast('新问题需要 API，请点击右上角配置。');}
  ...
  try{const payload={question:q,config:S.config,chunk_size:+$('chunkSize').value};if(S.graph)payload.graph=S.graph;else Object.assign(payload,{text:S.text,title:S.title});const j=await jsonPost('/api/run',payload);S.job=j.id;poll();}catch(e){finishJob();toast(e.message);$('answerState').textContent='请求未完成';}
};
...
$('jsonFile').onchange=async e=>{if(S.job)return toast('请先等待任务完成。');try{const file=e.target.files[0];if(!file)return;const data=JSON.parse(await file.text());hydrateGraph(data.graph||data);S.text='';S.result=null;S.liveTrace=[];renderSteps();$('presets').hidden=true;if(data.results?.length){S.demo=data;$('presets').innerHTML=data.results.map((r,i)=>`<option value="${i}">${i+1}. ${esc(r.question)}</option>`).join('');$('presets').hidden=false;selectPreset(0);}else{$('answerText').textContent='图谱已载入。配置 API 后可以提问。';$('evidenceList').replaceChildren();}toast('已载入图谱。');}catch(err){toast(err.message);}};
...
async function start(){if(window.__PUBLIC_DEMO__){$('importBtn').textContent='在本机使用';$('footerStatus').textContent='静态交互示例。新小说与 API 问答请在本机运行。';}if(window.__BOOT__){S.demo=window.__BOOT__;hydrateGraph(S.demo.graph);if(S.demo.results.length){$('presets').innerHTML=S.demo.results.map((r,i)=>`<option value="${i}">${i+1}. ${esc(r.question)}</option>`).join('');selectPreset(0);}htmlSource=document.documentElement.outerHTML; // Offline re-export uses the current self-contained file, handled below.
