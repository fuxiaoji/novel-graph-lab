...
`assets/project` 是可复制的精简项目模板，包含 core.py、server.py、cli.py、web、examples、tools 和 tests。第一次复制到新目录后，可以运行 `python server.py --open`。原 Demo 的 `examples/demo.json` 已经打包；如需单 HTML，可从网页导出，或调用 `tools.extract_demo.standalone`。

主要文件：

- `core.py`：分块、逐字引文验证、缓存、中英文 BM25、两轮图扩散、原文补充检索和引用检查。
